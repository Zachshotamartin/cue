...
async function job() {
  const p = createProject("Job recovery");
...
describe("durable provider jobs", () => {
  it("resumes a saved provider ID without submitting again", async () => {
    ...
    expect(getJob(j.id).state).toBe("running");
  });
  ...
  it("cancels a saved provider task, keeps its potential charge, and never resubmits", async () => {
    ...
    expect(getJob(j.id).chargedCents).toBe(25);
  });
  it("finishes a saved planner result without paying for another plan", async () => {
    vi.clearAllMocks();
    const p = createProject("Saved plan"),
      j = enqueue(p.id, "plan", { draft: p.draft }, crypto.randomUUID(), 25);
    await runJob(
      updateJob(j, {
        state: "running",
    ...
    expect(getJob(j.id).state).toBe("completed");
  });
